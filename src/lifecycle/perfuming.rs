...
pub fn perfume(
    ...
    let domains: std::collections::HashSet<&str> = impressions.iter().map(|i| i.domain.as_str()).collect();
        ...
        if count >= CRYSTALLIZATION_THRESHOLD {
            ...
            if existing.is_empty() {
                ...
                if let Some(pref_text) = summarize_impressions(&recent) {
                    let avg_valence: f32 = recent.iter().map(|i| i.valence).sum::<f32>() / recent.len() as f32;
                    ...
                    if let Some(p) = prefs.first() {
                        crate::store::strengths::init_strength(
                            conn,
                            NodeRef::Preference(p.id),
                        )?;
                    }
