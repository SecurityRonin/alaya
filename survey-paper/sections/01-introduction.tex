...
The urgency is measurable. OpenClaw, among the most widely deployed AI coding
agents, injects approximately 35,600 tokens of workspace files into
...
Over 150 memory systems have emerged since Park~\etal's seminal
...
Table~\ref{tab:survey-comparison} positions our work relative to the most
relevant prior surveys.

\begin{table}[!t]
\centering
\caption{Comparison with prior surveys on LLM agent memory.
$\bullet$~=~covered; $\circ$~=~partial; --~=~not covered.}
\label{tab:survey-comparison}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}lccccc@{}}
\toprule
& \rotatebox{70}{\textbf{CoALA~\cite{sumers2024coala}}}
& \rotatebox{70}{\textbf{Gao~\cite{gao2023rag}}}
& \rotatebox{70}{\textbf{Zhang~\cite{zhang2024survey}}}
& \rotatebox{70}{\textbf{Hu~\cite{hu2025memory}}}
& \rotatebox{70}{\textbf{Ours}} \\
\midrule
Systems surveyed      & ${\sim}$20 & -- & 40+ & 50+ & 150 \\
Production systems    & $\circ$ & -- & $\circ$ & $\bullet$ & $\bullet$ \\
Practitioner tools    & -- & -- & -- & -- & $\bullet$ \\
Memory structure      & $\bullet$ & -- & $\bullet$ & $\bullet$ & $\bullet$ \\
Retrieval taxonomy    & $\circ$ & $\bullet$ & $\circ$ & $\circ$ & $\bullet$ \\
Lifecycle operations  & -- & -- & $\bullet$ & $\bullet$ & $\bullet$ \\
Preference emergence  & -- & -- & -- & $\circ$ & $\bullet$ \\
Benchmark evaluation  & -- & -- & -- & $\circ$ & $\bullet$ \\
Neuro.\ grounding     & $\bullet$ & -- & $\circ$ & $\circ$ & $\bullet$ \\
\bottomrule
\end{tabular}
\end{table}
...
and dual-strength forgetting~\cite{bjork1992} predict structural
principles that appear to recur in agent memory architectures:
\textbf{use-dependent strengthening}, \textbf{dual-system
consolidation}, \textbf{associative spreading}, and \textbf{competitive
retrieval dynamics}. Whether this constitutes genuine convergence or
parallel coincidence is an interpretive claim we revisit in
\S\ref{sec:comparison}.

% Contributions
Our contributions are:
\begin{enumerate}[leftmargin=*, nosep]
    \item A four-axis taxonomy covering 150 memory systems across
    ...
    \item Documentation of the MEMORY.md problem and five recurring
    design patterns observed across independently developed systems.
