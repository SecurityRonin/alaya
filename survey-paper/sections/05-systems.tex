...
\paragraph{FadeMem.}
\textbf{FadeMem}~\cite{fademem2026} (Wei et al.) implements
biologically inspired dual-layer forgetting via adaptive exponential
decay functions modulated by three signals: semantic relevance, access
frequency, and temporal patterns.  The dual-layer hierarchy separates
active working memory from consolidated long-term storage, with
differential decay rates across layers.  Experiments on Multi-Session
Chat, LoCoMo, and LTI-Bench demonstrate 45\% storage reduction while
retaining 82.1\% of critical facts---the most favorable
storage-vs-retention tradeoff among forgetting-aware systems.  FadeMem
does not implement graph structure or contradiction resolution.

\paragraph{Oblivion.}
\textbf{Oblivion}~\cite{oblivion2026} (NEC Laboratories Europe)
decouples memory control into separate read and write paths.  The
\emph{read path} uses uncertainty-gating: the agent consults memory only
when its own uncertainty exceeds a threshold, avoiding unnecessary
retrieval on questions the model can answer from parametric knowledge
alone.  The \emph{write path} reinforces memories that contributed to
successful responses, strengthening useful associations over time.
Forgetting follows an Ebbinghaus-inspired decay model where memories
lose accessibility gradually without explicit deletion---a design that
preserves recoverability while reducing retrieval noise.  The
read-path uncertainty gate is architecturally unique among the surveyed
systems.

\paragraph{MaRS.}
\textbf{MaRS}~\cite{mars2026} (Alqithami) introduces privacy-aware
memory management with six theoretically grounded forgetting policies:
FIFO, LRU, Priority Decay, Reflection-Summary, Random-Drop, and a
Hybrid variant.  Each policy is analyzed for computational complexity
and sensitivity-aware retention, with optional $(\varepsilon,
\delta)$-differential privacy guarantees.  The Hybrid policy achieves a
0.911 composite score balancing performance, privacy, and efficiency on
the accompanying FiFA benchmark.  MaRS is the only surveyed system that
formalizes forgetting as a privacy mechanism with provable guarantees
rather than treating it solely as a storage optimization.

\paragraph{SuperLocalMemory V3.3.}
\textbf{SuperLocalMemory V3.3}~\cite{superlocalmemory2026} (Bhardwaj)
implements 7-channel cognitive retrieval combining multiple signal types
(semantic, lexical, temporal, frequency, entity, graph, and contextual)
with Ebbinghaus-inspired forgetting curves.  The system's defining
constraint is \emph{zero-LLM operation}: all memory management,
retrieval, and forgetting run without any LLM calls, using only local
models and classical algorithms.  This achieves 70.4\% on LoCoMo without
cloud dependencies---the highest score among systems that operate
entirely without LLM involvement.  The zero-LLM constraint makes
SuperLocalMemory suitable for privacy-sensitive and offline deployments
where no data can leave the local machine.
...
a multi-stage consolidation pipeline (near-duplicate merging,
working-to-episodic-to-semantic promotion), contradiction detection, and
confidence decay.  The consolidation pipeline is the most comprehensive
among coding agent memory systems: memories are progressively abstracted
from raw observations through episodic summaries to semantic knowledge,
with contradictions detected and resolved at each promotion stage.
AgentMemory achieves 96.2\% on LongMemEval, placing it among the highest
scores on this benchmark.  Built by a solo developer in 16 days and
released as open source, it demonstrates that a single-developer effort
can match or exceed the benchmark performance of team-built production
systems.

\textbf{Capy-Cortex}~\cite{capycortex2026} provides autonomous memory
for coding agents with a triple fusion retrieval pipeline combining FTS5
full-text search, TF-IDF scoring, and entity graph traversal.
Quality-gated storage filters incoming memories before persistence,
rejecting low-value or redundant entries.  A self-curating consolidation
process periodically merges related memories and prunes stale entries.
The combination of quality gating at write time and self-curation at
maintenance time makes Capy-Cortex one of the few coding agent memory
systems with both ingestion filtering and lifecycle management.
...
\paragraph{MAGMA.}
MAGMA~\cite{magma2026} (Jiang et al.) introduces a multi-graph
architecture that maintains four parallel graph views: semantic
(conceptual relationships), temporal (time-ordered event sequences),
causal (cause-effect chains), and entity (typed entities and
attributes).  An intent-aware routing layer analyzes incoming queries
and dispatches them to the relevant graph view(s), avoiding the
one-size-fits-all retrieval that characterizes most graph-based systems.
The multi-graph decomposition yields +45.5\% reasoning accuracy over
single-graph baselines and 95\% token reduction versus full-context
approaches.  MAGMA's architectural insight---that different query types
...
HippoRAG (which uses Personalized PageRank on a single knowledge
graph).

\paragraph{Mastra Observational Memory.}
Mastra's Observational Memory~\cite{mastra2026} extracts structured
\emph{observations} from agent interactions and stores them as
first-class memory objects rather than raw conversation transcripts.
The key finding is that observations outperform raw oracle data:
retrieving curated observations yields higher accuracy than retrieving
the original source material, suggesting that the extraction process
itself adds value through denoising and abstraction.  With Gemini as the
backbone, Mastra achieves 93.27\% on LongMemEval.  The system is
integrated with the Mastra agent framework, providing memory as a
built-in capability rather than a standalone service.  Mastra implements
no forgetting, graph structure, or consolidation beyond the initial
