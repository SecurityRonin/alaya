...
138~systems with complete capability data across the eight discriminating
features defined in \S\ref{sec:long-tail}.  Systems are grouped by category
and sorted alphabetically within each group.

\begin{table*}[!t]
\centering
\caption{Per-system capability matrix for 138 classified systems.
...
133 classified systems, while framework modules and coding tools remain
...
\begin{table}[!t]
\centering
\caption{Mean feature breadth by RAG tier for 133 classified systems
...
across the 133 classified systems.  Three correlations are notable.
First, graph structure and fusion are moderately co-occurrent (23 of 42
graph systems also use fusion, 55\%), confirming that graph traversal
provides an additional retrieval signal that motivates principled fusion.
Second, contradiction resolution is the rarest feature (14 systems) and
is almost always accompanied by semantic storage (13 of 14, 93\%)---consistent
with contradiction detection requiring a declarative fact store to compare
against.  Third, consolidation and forgetting partially co-occur (16
systems implement both), but the majority of consolidation systems
(27 of 43, 63\%) do not implement explicit forgetting, suggesting that
memory pruning and knowledge promotion are treated as orthogonal concerns
in many architectures.

\begin{table}[!t]
\centering
\caption{Feature co-occurrence matrix (lower triangle) for 133 classified
systems.  Diagonal (bold) shows total count per feature.  Notable:
Graph $\leftrightarrow$ Fusion = 23/42 (55\%);
Contradiction $\rightarrow$ Semantic = 13/14 (93\%).}
...
\midrule
Episodic      & \textbf{119} \\
Semantic      &  77 & \textbf{90} \\
Graph         &  32 & 40 & \textbf{42} \\
Fusion        &  34 & 38 & 23 & \textbf{40} \\
Consolidation &  43 & 37 & 16 & 15 & \textbf{43} \\
Forgetting    &  41 & 28 & 10 & 14 & 16 & \textbf{42} \\
Contradiction &  12 & 13 & 10 &  8 &  7 &  4 & \textbf{14} \\
Preference    &  26 & 18 &  6 & 11 &  6 & 12 &  2 & \textbf{27} \\
\bottomrule
\end{tabular}
\end{table}
...
The visualization includes the full 133-system matrix with sortable
