...
24 duplicates of already-included systems). The initial corpus comprised
138 systems. Continued monitoring through May~2026 identified 10
additional systems meeting inclusion criteria (including new production
deployments, RL-based memory architectures, and Claude ecosystem
additions), bringing the final corpus to 148~systems.
Figure~\ref{fig:prisma} visualizes the initial screening flow.
Table~\ref{tab:excluded} lists deliberate exclusions for systems that
required full-text review before exclusion. Table~\ref{tab:system-matrix}
provides each system's RAG tier and eight binary capability features for
the 148 systems with complete matrix records; the full four-axis coding
records in CSV and JSON are available in the supplementary
repository~\cite{alaya2026} under \texttt{survey-paper/data/}.
...
data extraction along four orthogonal axes.  The author coded each
system; a random 20\% sample ($n = 28$ systems) was independently
coded by a second coder (graduate research assistant) working from the
same operational definitions but without access to the primary coding.
Agreement was measured using Cohen's $\kappa$; values exceeded 0.85 on
all four axes (Structure: 0.91, Retrieval: 0.94, Lifecycle: 0.87,
Preference: 0.86).  Disagreements ($n = 12$ across the dual-coded
sample) were resolved by discussion until consensus; in two cases a
third annotator was consulted.  The dual-coded data and adjudication
log are available in the project repository~\cite{alaya2026}.  The four coding axes are:
...
May~2026, but significant new architectures continue to appear on a
