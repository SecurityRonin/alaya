...
\texttt{categories} (emergent ontology), and \texttt{tombstones} (cascade
deletion audit trail, recording node type, ID, deletion timestamp, and
reason for each purged node). Schema versioning uses SQLite's
\texttt{PRAGMA user\_version} (currently version~3) with forward migration
on database open.
...
\alaya{} follows test-driven development with 201 tests across four
categories: 161 deterministic unit tests covering individual store
operations, graph algorithms, lifecycle functions, and category CRUD;
13 property-based tests using \texttt{proptest} that verify mathematical
invariants (cosine similarity bounds, RRF score positivity and ordering,
Bjork strength clamping, Hebbian weight bounds, recency decay monotonicity);
9 integration tests covering full lifecycle pipelines (episode ingestion
through category emergence, including LTD decay, RIF suppression, and
tombstone tracking); and 18 documentation tests ensuring code examples in
the API documentation compile and produce correct results. Continuous
